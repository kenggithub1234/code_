...
class AutoLabelSegWorker(QThread):
    """Runs a trained ultralytics YOLO segmentation model over a list of
    frame indices, reading frames either from a video file or from a list of
    standalone image files, and emits detected polygons back to the GUI
    thread. Class matching/creation is intentionally NOT done here -
    detections are emitted as (class_name, [(x,y), ...]) and the receiving
    slot (running in the GUI thread) is responsible for mapping names to
    class ids and touching any Qt widgets, since it's unsafe to do that
    from a QThread."""

    progress_signal = pyqtSignal(int, int)          # (done, total)
    frame_result_signal = pyqtSignal(int, list)      # (frame_idx, [(name, [(x,y),...]), ...])
